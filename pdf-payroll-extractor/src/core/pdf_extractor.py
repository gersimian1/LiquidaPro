...
class RawEmployeeBlock:
    ...
    retroactivos_con_aporte: float = 0.0
    liquido: float = 0.0
...
class PDFExtractor:
    ...
    RE_RETROACTIVOS_CON_APORTE = re.compile(
        r'(?:DV\s+)?113260\s+Total Retroactivos con Aportes\s+([\d.]+,\d{2})'
    )
    ...
    def _parse_block(self, raw: str) -> Optional[RawEmployeeBlock]:
        ...
        m = self.RE_RETROACTIVOS_CON_APORTE.search(raw)
        block.retroactivos_con_aporte = self._parse_ar_number(m.group(1)) if m else 0.0
